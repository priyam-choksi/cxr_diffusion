...
def get_available_checkpoints():
    ...
    if not CHECKPOINTS_DIR.exists():
        st.warning("Model checkpoint directory not found. Please download model files.")
        # Return a dummy checkpoint that will trigger appropriate warnings later
        return {"best_model (download required)": "not_available"}
    ...
    # If no checkpoints found, provide a dummy entry
    if not ckpts:
        ckpts["best_model (download required)"] = "not_available"
        st.warning("No model checkpoints found. Please download model files.")
    
    # deterministic order
    ordered = ["best_model", "Epoch 40", "Epoch 480", "VAE best"]
    final_ckpts = {k: ckpts[k] for k in ordered if k in ckpts}
    
    # If still empty, add the dummy
    if not final_ckpts:
        final_ckpts = {"best_model (download required)": "not_available"}
...
def load_model(model_path):
    ...
    try:
        ...
        if not Path(model_path).exists():
            return None, "Model files not found. Please download and add them to the appropriate directory."
...
def load_dataset_sample():
    ...
    try:
        ...
        if not image_path.exists() or not reports_csv.exists() or not projections_csv.exists():
            return None, "Dataset files not found. Please download and add them to the appropriate directory."
...
def run_generator_mode(model_path):
    ...
    if generate_pressed:
        ...
        generator, message = load_model(model_path)
        if generator is None:
            progress_bar.progress(100)
            status.error(f"❌ {message}")
            st.warning("Model files are not available. Please download the model files and add them to the appropriate directory structure.")
            return
