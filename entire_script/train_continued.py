"""
Use this after Gan_1 to further tweak and train diffusion model!
"""
...
# Add this near the top of your configuration section

# Path configuration
DATASET_PATH = r'F:\DeepLearning Course\Project\gan'
IMAGES_PATH = os.path.abspath(os.path.join(DATASET_PATH, 'dataset/images/images_normalized/'))
REPORTS_CSV = os.path.join(DATASET_PATH, 'dataset/indiana_reports.csv')
PROJECTIONS_CSV = os.path.join(DATASET_PATH, 'dataset/indiana_projections.csv')
...
CHECKPOINT_FREQ = 5  # Save every 10 epochs
KEEP_LAST_CHECKPOINTS = 50  # Only keep 2 most recent checkpoints
...
def train_diffusion(model, train_loader, val_loader, optimizer, device, num_epochs=50, checkpoint_dir=None, use_amp=True, train_unet_only=True, tokenizer=None, save_checkpoints=True, start_epoch=0):  # Add start_epoch here    """Train diffusion model."""
...
def main():
    ...
    # Skip VAE training and load from checkpoint instead
    logger.info("Loading pre-trained VAE from checkpoint")
    vae_checkpoint_dir = os.path.join(OUTPUT_DIR, "vae_checkpoints")
    vae_checkpoint_path = os.path.join(vae_checkpoint_dir, "best_model.pt")

    try:
        checkpoint = torch.load(vae_checkpoint_path, map_location=device)
        if 'model_state_dict' in checkpoint:
            vae.load_state_dict(checkpoint['model_state_dict'])
            logger.info("Successfully loaded VAE weights from checkpoint")
        else:
            logger.error("Checkpoint doesn't contain model_state_dict. Check the checkpoint format.")
            raise ValueError("Invalid checkpoint format")
    except Exception as e:
        logger.error(f"Error loading VAE checkpoint: {e}")
        import traceback
        logger.error(traceback.format_exc())
        raise RuntimeError("Failed to load VAE checkpoint")
    ...
    logger.info("Starting diffusion model training")
    diffusion_checkpoint_dir = os.path.join(OUTPUT_DIR, "diffusion_checkpoints_continued")
    ...
    # # Load existing diffusion checkpoint
    # diffusion_checkpoint_path = os.path.join(OUTPUT_DIR, "diffusion_checkpoints", "best_model.pt")
    # logger.info(f"Loading diffusion checkpoint from {diffusion_checkpoint_path}")
    
    # Load existing diffusion checkpoint post 200 epochs
    diffusion_checkpoint_path = os.path.join(OUTPUT_DIR, "diffusion_checkpoints_continued", "checkpoint_epoch_350.pt")
    logger.info(f"Loading diffusion checkpoint from {diffusion_checkpoint_path}")
    ...
    try:
        ...
        if 'unet_state_dict' in checkpoint:
            diffusion_model.unet.load_state_dict(checkpoint['unet_state_dict'])
            logger.info("Successfully loaded UNet weights")
        
        if 'text_encoder_state_dict' in checkpoint:
            diffusion_model.text_encoder.load_state_dict(checkpoint['text_encoder_state_dict'])
            logger.info("Successfully loaded text encoder weights")
        
        if 'optimizer_state_dict' in checkpoint:
            diffusion_optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        ...
        logger.error(f"Error loading diffusion checkpoint: {e}")
        logger.warning("Will continue with fresh diffusion training")
...
# At the very end of your current code cell (just before or after the if __name__ == "__main__" block),
# add these lines:
